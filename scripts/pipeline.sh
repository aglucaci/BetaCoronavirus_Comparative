...
if [ -s $BASEDIR"/analysis/blastn_results/SARS2/blastn_QA_filtered/ORF10.fasta" ];
then
    echo 1 
...
# Step 2b - ModelTest
echo qsub -V -q epyc -W depend=afterok:$jobid_2a run_modeltest.sh
cmd="qsub -V -q epyc -W depend=afterok:$jobid_2a run_modeltest.sh"
...
echo qsub -V -W depend=afterok:$jobid_2a -q epyc -N tn93_codons tn93_codons.sh -F $BASEDIR
cmd="qsub -V -W depend=afterok:$jobid_2a -q epyc -N tn93_codons tn93_codons.sh -F $BASEDIR"
jobid_3a=$($cmd | cut -d' ' -f3)
echo "Step 3a: "$jobid_3a
echo ""

# Step 3b - TN93 Cluster analysis
echo qsub -V -W depend=afterok:$jobid_2a -q epyc tn93_cluster.sh -F $BASEDIR
cmd="qsub -V -W depend=afterok:$jobid_2a -q epyc tn93_cluster.sh -F $BASEDIR"
jobid_3b=$($cmd | cut -d' ' -f3)
echo "Step 3b: "$jobid_3b
echo ""

# Step 4 - Selection Analyses
echo qsub -V -W depend=afterok:$jobid_2c -q epyc -l nodes=1:ppn=28 MEME_BUSTEDS_aBSREL.sh -F $BASEDIR
cmd="qsub -V -W depend=afterok:$jobid_2c -q epyc -l nodes=1:ppn=28 MEME_BUSTEDS_aBSREL.sh -F $BASEDIR"
jobid_4=$($cmd | cut -d' ' -f3)
echo "Step 4: "$jobid_4
echo ""

# Step 5 - use MAFFT Merge to merge the alignments across genes, and then construct ML Trees.
echo qsub -V -W depend=afterok:$jobid_2a -q epyc MergeAlignments.sh -F $BASEDIR
cmd="qsub -V -W depend=afterok:$jobid_2a -q epyc MergeAlignments.sh -F $BASEDIR"
...
echo qsub -V -W depend=afterok:$jobid_5 -q epyc launcher_for_annotator.sh -F $BASEDIR 
cmd="qsub -V -W depend=afterok:$jobid_5 -q epyc launcher_for_annotator.sh -F $BASEDIR"
jobid_6=$($cmd | cut -d' ' -f3)
echo "Step 6: "$jobid_6
echo ""

# Step 7 - Run RELAX and Constrast-FEL
echo qsub -V -W depend=afterok:$jobid_5 -q epyc RELAX_CONTRAST-FEL.sh -F $BASEDIR
cmd="qsub -V -W depend=afterok:$jobid_5 -q epyc RELAX_CONTRAST-FEL.sh -F $BASEDIR"
